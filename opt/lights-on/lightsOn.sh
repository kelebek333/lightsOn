...
setPresentationMode()
{
	if [ $xfce4powermanager_control == 1 ]; then
		...
		xfconf-query -c xfce4-screensaver -p /saver/fullscreen-inhibit -s $1
	fi	
}
...
isAppRunning()
{
    ...
    if [ $celluloid_detection == 1 ]; then runcheck celluloid; if [ $? == 1 ]; then return 1; fi; fi
